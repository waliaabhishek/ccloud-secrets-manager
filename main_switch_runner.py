...
wf_args.add_argument(
    ...
    action="append",
    dest="wf_name",
    choices=["create-sa", "get-sa", "create-apikey"],
    help="Workflow that will be executed",
)
...
sm_args.add_argument(
    "--rest-proxy-secret-name-identifier",
    type=str,
    metavar="rest-proxy-user",
    required="--enable-connect-rest-source" in argv,
    help="The REST proxy identifier string in Secret Manager.",
)
sm_args.add_argument(
    "--rest-proxy-basic-secret-file-path",
    type=str,
    metavar="/mnt/secrets/k8sSecretName/basic.txt",
    required="--enable-connect-rest-source" in argv,
...
if "create-sa" == args.wf_name:
    sa_output = service_account.run_sa_workflow(args.service_account_name, True, args.force_new_account)
if "get-sa" == args.wf_name:
    sa_output = service_account.run_sa_workflow(args.service_account_name, False, False)
if "create-apikey" == args.wf_name:
    existing_api_key_output, new_api_key_output = api_key_manager.run_api_key_workflow(
        ...
        args.create_service_account_if_necessary,
        True,
        args.cluster_list,
